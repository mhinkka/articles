 ...
 */
public final class TesterCaching {
	...
	if (numRepeats > 0) {
		...
		System.out.println(new Date().toString() + " Load skipped in caching test...");
	    }
	    else {
		load();
    ...
    {
	Date start, end;
	System.out.println((start = new Date()).toString() + " Starting spark warm-up operation: " + testName);

	if (testName.equals("flows"))
	    flows();
	else if (testName.equals("variations")) 
	    variations();
	end = new Date();
	System.out.println("Finished spark warm-up operation: " + testName + " " + (end.getTime() - start.getTime()) + " ms");

	for (int i = 0; i < numRepeats; ++i) {
	    System.out.println((start = new Date()).toString() + " Starting spark operation: " + testName);
	    if (testName.equals("flows"))
		flows();
	    else if (testName.equals("variations")) 
		variations();
	    System.out.println("Finished spark operation: " + testName + " " + ((end = new Date()).getTime() - start.getTime()) + " ms");
	}
    }
	...
	schemaEvents.persist(StorageLevel.MEMORY_ONLY_SER());

	System.out.println(new Date().toString() + " Data loaded to spark");
    }
