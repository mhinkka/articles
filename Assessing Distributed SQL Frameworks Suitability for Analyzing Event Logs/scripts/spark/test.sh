...
rm -f $SPARK_HOME
ln -s $HOME/spark/spark-1.0.2-bin-hadoop2 $SPARK_HOME

echo "Using spark from: $HOME/spark/spark-1.0.2-bin-hadoop2"
...
withTimeout /usr/bin/time -o $OUT_DIR/load-results.txt -a $SPARK_HOME/bin/spark-submit --executor-memory 16G --total-executor-cores 100 --class $TEST_CLASS script/Tester.jar load $OUT_DIR 1 $OUT_DIR/parquet >> $OUT_DIR/load-results.txt
date >> $OUT_DIR/load-results.txt
...
withTimeout /usr/bin/time -o $OUT_DIR/warm-up-results.txt -a $SPARK_HOME/bin/spark-submit --executor-memory 16G --total-executor-cores 100 --class $TEST_CLASS script/Tester.jar $TEST_NAME $OUT_DIR 0 $OUT_DIR/parquet >> $OUT_DIR/warm-up-results.txt
...
do
  ...
  withTimeout /usr/bin/time -o $out/results.txt -a $SPARK_HOME/bin/spark-submit --executor-memory 16G --total-executor-cores 100 --class $TEST_CLASS script/Tester.jar $TEST_NAME $OUT_DIR $NUM_REPEATS $OUT_DIR/parquet >> $out/results.txt
